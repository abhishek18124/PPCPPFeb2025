...
string minWindow(string& s, string& t) {
	...
	int tCnt = tFreqMap.size(); // to store the no. of unique characters in 't'.
	...
	while (j < m) {
		...
		// expand the window i.e. add s[j] to the window
			...
			while (tCnt == 0) {
				...
				if (tFreqMap.find(out) != tFreqMap.end()) {
					...
					if (tFreqMap[out] == 1) {
						...
						i++;
						tCnt++; // since tCnt becomes 1 you'll automatically exit from the shrinking loop and start expanding
					} else {
