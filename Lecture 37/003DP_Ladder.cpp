...
int f(int n, int k) {
	...
	for (int j = 1; j <= k; j++) {
		...
		if (n - j >= 0) { // yes, you can reach the nth step from the n-jth step,
...
// time : O(n)
// space: O(n) due to dp[]
// [HW] try to optimise the space to O(k)
...
int fBottomUpTimeOptimised(int n, int k) {
	...
	dp[0] = 1; // at the 0th index of dp[] we store f(0)
	dp[1] = dp[0]; // at the 1st index of dp[] we store f(1)
